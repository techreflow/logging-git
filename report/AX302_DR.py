...
url = 'https://malladi.s3.amazonaws.com/AX302.csv'
...
# Define separate thresholds for deviations
deviation_thresholds = {sensor_columns[0]: 9}  # Column 1 threshold
deviation_thresholds.update({col: 7 for col in sensor_columns[1:]})  # All other columns
...
for i, col in enumerate(sensor_columns):
    ...
    # Get the deviation threshold for the current column
    deviation_threshold = deviation_thresholds.get(col, 8)
...
def send_email_with_attachment(report_path):
    ...
    msg = MIMEMultipart()
    msg['From'] = user
    msg['To'] = ', '.join(['vendhan@malladi.co.in','sabari@malladi.co.in','gabriel@malladi.co.in'])
    msg['Subject'] = 'M3 Pressure Deviation Report'
...
# Clean up: Delete the plot images
